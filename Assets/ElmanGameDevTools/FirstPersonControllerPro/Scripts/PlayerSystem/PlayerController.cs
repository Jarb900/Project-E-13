...
namespace ElmanGameDevTools.PlayerSystem
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        [Header("ANIMATION FOLLOW")]
        [Tooltip("The transform of your character's Head bone")]
        ...
        void Start()
        {
            ...
            animStateLand = Animator.StringToHash("Land");

            if (headBone != null) initialHeadY = headBone.localPosition.y;
        }
        ...
        void Update()
        {
            ...
            // Tell the Animator if we are grounded or not (for falling animations)
            // We use IsEffectivelyGrounded() to include coyote time, which looks more natural
            if (anim != null) anim.SetBool("IsGrounded", IsEffectivelyGrounded() || isCrouching);
        ...
        private void HandleMovement()
        {
            ...
            if (anim != null)
            {
                // Calculate the speed for the animator
                float animSpeed = 0f;

                // Check for input magnitude (is the player pressing move keys?)
                if (move.magnitude > 0.1f)
                {
                    // If moving, check if running or walking
                    animSpeed = (currentMovementState == MovementState.Running) ? 1.0f : 0.5f;
                }

                // Override for crouching
                if (isCrouching)
                {
                    animSpeed = (move.magnitude > 0.1f) ? 0.5f : 0f;
                }

                // Send the final speed to the Animator's "Speed" parameter
        ...
        {
            // --- 1. Calculate the base height from crouching ---
            float camHeight = cameraBaseHeight * (controller.height / originalHeight);

            // --- 2. Calculate the dip from the head's animation ---
            float headDip = 0f;
            if (headBone != null)
            {
                float currentHeadY = headBone.localPosition.y;
                headDip = currentHeadY - initialHeadY;
            }

            // --- 3. Combine them ---
            float targetCamHeight = camHeight + headDip;

            // --- 4. Smooth the Y-axis movement ---
            float newCamHeight = Mathf.SmoothDamp(playerCamera.localPosition.y, targetCamHeight, ref cameraHeightVelocity, 0.05f);

            // --- 5. Apply ALL changes (Z-axis is now handled by camera position) ---
            playerCamera.localPosition = new Vector3(
                playerCamera.localPosition.x,
                newCamHeight, // The new Y-axis (with crouch AND animation)
                0.25f         // Set this to your preferred "resting" Z-position
            );
        }
